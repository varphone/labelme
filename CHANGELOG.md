...
- Changed the file list to sort by file name with a plain, case-insensitive lexicographic order that is identical on every platform and locale; previously the locale-aware collation reordered ASCII names surprisingly under locales such as `zh_CN`, and the later natural order treated leading zeros numerically, so hex-digest names like `02be..` landed after `2bd6..`. Dropped image files are now sorted into the list as well, instead of being appended in drop order ([#2482](https://github.com/wkentaro/labelme/pull/2482))
- Changed the "Remove Selected Point" shortcut from `Meta+H` to `X` (`Backspace` is kept as a second binding) because `Meta+H` is captured by the system on several Linux desktops ([#2486](https://github.com/wkentaro/labelme/pull/2486))
