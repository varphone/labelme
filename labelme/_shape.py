...
def nearest_edge_index(
    *,
    ...
    if shape.shape_type in BEZIER_SHAPE_TYPES:
        ...
        return _nearest_index_within_epsilon(
    ...
    if shape.shape_type in SPLINE_SHAPE_TYPES:
        ...
        if shape.shape_type == "bspline":
            ...
            control_index = int(
                np.argmin(
                    np.linalg.norm(point - control_projections, axis=1)
                )
            )
